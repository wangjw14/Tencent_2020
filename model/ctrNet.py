...
class ctrNet(nn.Module):
    ...
    def train(self,train_dataset,dev_dataset=None):
        ...
        #设置优化器
        optimizer = torch.optim.Adam(self.model.parameters(), lr=args.lr, eps=1e-8,weight_decay=1e-4)
        # optimizer = torch.optim.Adam(self.model.parameters(), lr=config.learning_rate,weight_decay=1e-4) 
        ...
        global_step = 0
        best_age_acc, best_gender_acc, avg_loss =  0.0, 0.0, 0.0
        model.zero_grad()  
        for idx in range(args.num_train_epochs):     
            tr_num=0
            train_loss=0
            logger.info("epoch {} ".format(idx))
            for step, batch in enumerate(train_dataloader):
                #forward和backward
                age, gender, dense_features, text_features=(x.to(args.device) for x in batch)  
                del batch
                model.train()
                loss = model(age, gender, dense_features, text_features) 
                if args.n_gpu > 1:
                ...
                if (step+1) % args.display_steps == 0:
                    logger.info("  epoch {} step {} loss {} avg loss {}".format(idx,step+1,loss.item(),avg_loss))
                #update梯度
                optimizer.step()
                optimizer.zero_grad()
                # scheduler.step()  
    ...
    def infer(self,eval_dataset):
        ...
        for batch in eval_dataloader:       
            age, gender, dense_features, text_features=(x.to(args.device) for x in batch) 
            with torch.no_grad():
                probs_1,probs_2 = model(None, None, dense_features, text_features)  
